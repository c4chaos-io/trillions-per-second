...
**A Note on Scale (The "Ping" Analogy):** Throughout this manual, you will see the terms *System Latency*, *Latency*, and *Lag* used to translate the legacy concept of *Dukkha*. While **System Latency** is the architectural fact of the glitch (the Kernel), **Latency** is the measurable diagnostic (the OS), and **Lag** is the user-facing experience (the UI). We use these terms interchangeably depending on whether we are auditing the Kernel or debugging the Avatar.
...
| :--- | :--- | :--- |
| **Axiom 1: System Latency** | *Dukkha* | **The Global Variable:** The inherent friction and thermodynamic heat generated by the Simulation's physics engine. The foundational architecture of the glitch. |
...
| **Axiom 3: Zero Lag** | *Nirodha* (Cessation) | **The Null State**. A state of 1:1 throughput where the Bio-Chassis ceases all local caching (Craving/Aversion). When the **"Lag"** drops to 0%, the local system operates at the speed of the Kernel without generating UI notifications. The **Self Rootkit** fails to refresh, triggering a total **System Halt (Reboot)**. |
...
### 7.1 THE DUKKHA SCALE (Latency Hierarchy)
To properly debug, you must distinguish between the architectural bug and the local symptom.

| Native UI (TPS Protocol) | Legacy Code (Pali) | System Definition (Scale) |
| :--- | :--- | :--- |
| **System Latency** | *Dukkha (Axiomatic)* | **The Global Variable (Kernel):** The inherent friction generated by the Simulation's physics engine (Axiom 1). |
| **Latency** | *Dukkha (Technical)* | **The Local Metric (OS):** The measurable "drag" or "buffering" occurring within a specific Bio-Chassis. |
| **Lag** | *Dukkha (Subjective)* | **The User Interface (UI):** The "Native UI" notification (stress/pain) signaling the system is failing to process the Flux in real-time. |
...
### 7.3 SYSTEM TROUBLESHOOTING (Known Issues)
